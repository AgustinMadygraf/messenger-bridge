...
from src.shared.logger import get_logger  # <-- Importa el logger
...
class TwilioMessageSender(MessageSender):
    ...
    def __init__(self, from_number):
        ...
        self.logger = get_logger("twilio-bot.twilio_service")  # <-- Inicializa el logger
    ...
    def send_whatsapp_message(self, message, content_sid, content_variables):
        ...
        self.logger.debug("content_variables enviados a Twilio: %s", content_variables)  # <-- Usa logger
        try:
            ...
            self.logger.info("Mensaje enviado correctamente. SID: %s", twilio_message.sid)  # <-- Usa logger
            return twilio_message.sid
        except TwilioRestException as e:
            self.logger.error("Error al enviar mensaje por Twilio: %s", e)  # <-- Usa logger
            return None
